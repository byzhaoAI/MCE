...
def run_model(model, loss_fn, d, device='cuda:0', mask=None, sw=None, is_training=True):
    metrics = {}
...
def main(
    ...
    missing_rate = _missing_rates[missing_id]
    print(_missing_rates, missing_rate)
    ...
    ckpt_path = os.path.join(ckpt_dir, f'ckpt_{id_list[0]}{id_list[1]}{id_list[2]}.pth')
    logging.basicConfig(
        level = logging.INFO,
        format = '%(asctime)s - %(levelname)s - %(message)s',
        filename = os.path.join(log_dir, f'{model_name}_{id_list[0]}{id_list[1]}{id_list[2]}.log'),  # 指定文件名
    ...
    model = Segnet(Z, Y, X, vox_util, use_camera=use_camera, use_radar=use_radar, use_lidar=use_lidar, use_metaradar=use_metaradar, do_rgbcompress=do_rgbcompress, encoder_type=encoder_type, rand_flip=rand_flip)
    ...
    while global_step < max_iters:
        ...
        if do_val:
            logging.info('%s; step %06d/%d; rtime %.2f; itime %.2f; loss %.5f; iou_t %.1f; iou_v %.1f' % (
    ...
    logging.info('validate the final model...')
    for _idx, _mode in enumerate(full_mode_list):
        ...
        for _, sample in enumerate(val_dataloader): 
            with torch.no_grad():
                _, metrics = run_model(model, seg_loss_fn, sample, device, mask=_mode, is_training=False)
        ...
        # sum_UB, avg_UB = np.sum(UB), np.mean(UB) / B
        # sum_SM, avg_SM = np.sum(SM), np.mean(SM) / B
        iou = np.mean(iou)
        ...
        best_ckpt_path = os.path.join(ckpt_dir, f'best{_mode[0]}{_mode[1]}{_mode[2]}', f'ckpt_{id_list[0]}{id_list[1]}{id_list[2]}.pth')
