...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = (
            self.qkv(x)
            .reshape(B, N, 3, num, self.num_heads, C // self.num_heads)
            .permute(2, 0, 1, 4, 3, 5)
        )
        q, k, v = (
            qkv[0],
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, num, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class Transformer(nn.Module):
    def __init__(self, embedding_dim=320, depth=1, heads=8, mlp_dim=1024, dropout_rate=0.1):
