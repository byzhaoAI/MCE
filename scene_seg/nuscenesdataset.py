...
class NuscData(torch.utils.data.Dataset):
    def __init__(self, nusc, is_train, data_aug_conf, centroid=None, bounds=None, res_3d=None, nsweeps=1, seqlen=1, refcam_id=1, get_tids=False, temporal_aug=False, use_radar_filters=False, do_shuffle_cams=True, missing_rate=None):
        self.nusc = nusc
...
class VizData(NuscData):
    def __init__(self, *args, **kwargs):
        ...
        if self.missing_rate is not None:
            import pandas as pd
            rate_name = f'{int(self.missing_rate[0]*10)}{int(self.missing_rate[1]*10)}{int(self.missing_rate[2]*10)}'
            df = pd.read_csv(f"train_split/nuScenes_imb_{rate_name}.csv")
            self.mask = torch.from_numpy(df.iloc[:, :-1].to_numpy(dtype=int))
        else:
            self.mask = torch.ones((self.indices.shape[0], 3))
    ...
    def get_single_item(self, index, cams, refcam_id=None):
        ...
        if self.get_tids:
            return mask, imgs, rots, trans, intrins, lidar0_data, lidar0_extra, lidar_data, lidar_extra, lrtlist, vislist, tidlist_, scorelist, seg_bev, valid_bev, center_bev, offset_bev, size_bev, ry_bev, ycoord_bev, radar_data, egopose
        else:
            return mask, imgs, rots, trans, intrins, lidar0_data, lidar0_extra, lidar_data, lidar_extra, lrtlist, vislist, scorelist, seg_bev, valid_bev, center_bev, offset_bev, size_bev, ry_bev, ycoord_bev, radar_data, egopose
    ...
    def __getitem__(self, index):
        ...
        for index_t in self.indices[index]:
            ...
            mask, imgs, rots, trans, intrins, lidar0_data, lidar0_extra, lidar_data, lidar_extra, lrtlist, vislist, tidlist, scorelist, seg_bev, valid_bev, center_bev, offset_bev, size_bev, ry_bev, ycoord_bev, radar_data, egopose = self.get_single_item(index_t, cams, refcam_id=refcam_id)
        ...
        return all_masks, all_imgs, all_rots, all_trans, all_intrins, all_lidar0_data, all_lidar0_extra, all_lidar_data, all_lidar_extra, all_lrtlist, all_vislist, all_tidlist, all_scorelist, all_seg_bev, all_valid_bev, all_center_bev, all_offset_bev, all_radar_data, all_egopose
...
def missing_pattern(num_modality, num_sample, ratio):
    missing_matrix = torch.ones((num_sample, num_modality))
    for i in range(0, num_modality):
        missing_index = torch.randperm(num_sample)[:int(num_sample * ratio[i])]
