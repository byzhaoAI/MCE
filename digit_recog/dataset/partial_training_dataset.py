...
class MetaTrSouMNIST(torch.utils.data.Dataset):
	...
	def __init__(self, img_root, sound_root, per_class_num=50, meta_split='mtr'):
		...
		self.only_image_list = self.get_meta_train_image_list(self.img_root,self.per_class_num)
			
		self.full_image_list = self.get_meta_val_image_list(self.img_root,self.per_class_num)
		self.full_sound_list = self.get_meta_val_sound_list(self.sound_root,self.per_class_num)

		self.all_image_list = self.full_image_list + self.only_image_list
		print('full, partial: ', len(self.full_image_list), len(self.only_image_list))
	...
	def __getitem__(self, index):
		...
		if index < len(self.full_image_list):
			sound_path = self.full_sound_list[index]
			...
			sound = torch.zeros(1, 20, 20)
			mask = torch.tensor([1, 0])
		
		label = torch.tensor(image_label).long()
		
		return im, sound, mask, label
